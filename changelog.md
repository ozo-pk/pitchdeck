...
- **Light Mode UI/UX Redesign:** Completely overhauled the frontend design across the Leaderboard, Student Portal, Judge Panel, and Admin Dashboard. Transitioned from a dark-mode theme to a premium SaaS aesthetic using `slate-50` backgrounds, high-contrast typography, and smooth glassmorphic elements. Introduced micro-interactions, CSS keyframe animations (like `fade-in-up`), and refined card layouts without altering any backend JS functionality.
